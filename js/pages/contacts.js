import { relationshipsUsersGet } from '../api.js';
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    async function getUsersList() {
        try {
            const result = await relationshipsUsersGet(currentUserId);

            if (result.success) {
                updateCategoryUsers(result.users['mutual'], 'mutual');
                updateCategoryUsers(result.users['outgoing'], 'outgoing');
                updateCategoryUsers(result.users['incoming'], 'incoming');
                return;
            } else {
